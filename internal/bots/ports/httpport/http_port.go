...
func (h Server) CreateBot(w http.ResponseWriter, r *http.Request) {
	postBots := PostBots{}
	...
	err := h.app.Commands.CreateBot.Handle(r.Context(), command.CreateBot{
...
func (h Server) GetBot(w http.ResponseWriter, r *http.Request, uuid string) {
	bot, err := h.app.Queries.GetBot.Handle(r.Context(), query.GetBot{
		BotUUID: uuid,
	})
	if errors.As(err, &interfaces.BotNotFoundError{}) {
...
func (h Server) GetAnswers(w http.ResponseWriter, r *http.Request, uuid string) {
	answers, err := h.app.Queries.AllAnswers.Handle(r.Context(), query.GetAnswersTable{
		BotUUID: uuid,
	})
	if errors.As(err, &interfaces.BotNotFoundError{}) {
		http.Error(w, err.Error(), http.StatusNotFound)
